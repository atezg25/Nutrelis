  ...
  return (
    <HydrationGuard bg="#fff" style={{ color: "#1a1a1a", overflowX: "hidden" }}>
      ...
      <section style={{ background: "linear-gradient(135deg, #060f08, #0a1a0d)", padding: isMobile ? "48px 16px" : isSmall ? "56px 24px" : "72px 60px", textAlign: "center" }}>
        ...
        <p style={{ color: "rgba(255,255,255,0.6)", fontSize: isMobile ? 14 : 16 }}>{t("shipping.heroDesc")}</p>
      </section>

      {/* OPTIONS LIVRAISON */}
      <section style={{ padding: `${isMobile ? "48px" : isSmall ? "64px" : "80px"} ${px}` }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <h2 style={{ fontFamily: "var(--font-sora), sans-serif", fontSize: isMobile ? "1.3rem" : isSmall ? "1.7rem" : "1.8rem", fontWeight: 800, marginBottom: isMobile ? 28 : 48, textAlign: "center" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? 16 : 24 }}>
            {[
              ...
              <div key={i} style={{ background: opt.popular ? "#f0faf2" : "#f8f9fa", border: opt.popular ? "2px solid var(--accent)" : "1px solid #e0e0e0", borderRadius: isMobile ? 14 : 20, padding: isMobile ? 20 : 36, position: "relative" }}>
                {opt.popular && (
                ...
                <div style={{ fontSize: isMobile ? 28 : 36, marginBottom: 12 }}>{opt.icon}</div>
                <h3 style={{ fontFamily: "var(--font-sora), sans-serif", fontSize: isMobile ? "1.05rem" : "1.2rem", fontWeight: 800, marginBottom: 8 }}>{opt.titre}</h3>
                <div style={{ color: "var(--accent)", fontSize: isMobile ? "1.3rem" : "1.6rem", fontWeight: 900, marginBottom: 4 }}>{opt.prix}</div>
                <div style={{ color: "#888", fontSize: 13, marginBottom: isMobile ? 14 : 20 }}>{opt.prixDesc}</div>
                <div style={{ background: "var(--accent)", color: "#060f08", display: "inline-block", padding: isMobile ? "5px 12px" : "6px 16px", borderRadius: 20, fontSize: isMobile ? 12 : 14, fontWeight: 700, marginBottom: isMobile ? 16 : 24 }}>
                  {"⏱️ " + opt.delai}
                </div>
                <ul style={{ listStyle: "none", padding: 0, display: "flex", flexDirection: "column", gap: isMobile ? 8 : 10 }}>
                  {opt.details.map((d, j) => (
                    <li key={j} style={{ display: "flex", alignItems: "center", gap: 10, fontSize: isMobile ? 13 : 14, color: "#444" }}>
      ...
      {/* PROCESSUS */}
      <section style={{ padding: `${isMobile ? "48px" : isSmall ? "64px" : "80px"} ${px}`, background: "#f0faf2" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
            {[
              ...
              <div key={i} style={{ display: "flex", gap: isMobile ? 14 : 24, padding: isMobile ? "20px 0" : "28px 0", borderBottom: i < 4 ? "1px solid #c8e6d0" : "none" }}>
                <div style={{ width: isMobile ? 38 : 48, height: isMobile ? 38 : 48, borderRadius: "50%", background: "var(--accent)", display: "flex", alignItems: "center", justifyContent: "center", color: "#060f08", fontWeight: 900, fontSize: isMobile ? 13 : 15, flexShrink: 0, fontFamily: "var(--font-sora), sans-serif" }}>
                  {step.num}
                </div>
                <div>
                  <div style={{ fontWeight: 800, fontSize: isMobile ? 14 : 16, marginBottom: 6 }}>{step.titre}</div>
                  <div style={{ color: "#555", fontSize: isMobile ? 13 : 14, lineHeight: 1.6 }}>{step.desc}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ padding: `${isMobile ? "48px" : isSmall ? "64px" : "80px"} ${px}` }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <h2 style={{ fontFamily: "var(--font-sora), sans-serif", fontSize: isMobile ? "1.3rem" : isSmall ? "1.7rem" : "1.8rem", fontWeight: 800, marginBottom: 16, textAlign: "center" }}>
            {t("shipping.returnsTitle")}
          </h2>
          <p style={{ color: "#555", fontSize: isMobile ? 14 : 16, lineHeight: 1.8, textAlign: "center", maxWidth: 680, margin: isMobile ? "0 auto 28px" : "0 auto 48px" }}>
            {t("shipping.returnsDesc")}
          </p>

          <div style={{ background: "#f0faf2", border: "2px solid var(--accent)", borderRadius: isMobile ? 14 : 20, padding: isMobile ? "24px 16px" : "40px", marginBottom: isMobile ? 16 : 28 }}>
            <div style={{ display: "flex", gap: isMobile ? 12 : 20, alignItems: "flex-start" }}>
              <span style={{ fontSize: isMobile ? 28 : 40 }}>🛡️</span>
              <div>
                <h3 style={{ fontFamily: "var(--font-sora), sans-serif", fontSize: isMobile ? "1.05rem" : "1.2rem", fontWeight: 800, marginBottom: 12, color: "var(--accent)" }}>
                  {t("shipping.guaranteeTitle")}
                </h3>
                <p style={{ color: "#444", fontSize: isMobile ? 13 : 15, lineHeight: 1.8, marginBottom: isMobile ? 14 : 20 }}>
                ...
                <div style={{ display: "flex", gap: isMobile ? 8 : 12, flexWrap: "wrap" }}>
                  {[t("shipping.guaranteeBadge1"), t("shipping.guaranteeBadge2"), t("shipping.guaranteeBadge3"), t("shipping.guaranteeBadge4")].map((b, i) => (
                    <span key={i} style={{ background: "#fff", border: "1px solid #c8e6d0", padding: isMobile ? "4px 10px" : "6px 14px", borderRadius: 20, fontSize: isMobile ? 11 : 13, fontWeight: 600, color: "#333" }}>{"✅ " + b}</span>
                  ))}
                </div>
              </div>
            </div>
          ...
          <div style={{ background: "#f8f9fa", border: "1px solid #e0e0e0", borderRadius: isMobile ? 12 : 16, padding: isMobile ? 16 : 28 }}>
            <h3 style={{ fontWeight: 800, fontSize: isMobile ? 14 : 15, marginBottom: 16 }}>{t("shipping.refundTitle")}</h3>
            <ol style={{ paddingLeft: 20, display: "flex", flexDirection: "column", gap: isMobile ? 8 : 10, color: "#555", fontSize: isMobile ? 13 : 14, lineHeight: 1.6 }}>
      ...
      {/* FAQ RAPIDE */}
      <section style={{ padding: `${isMobile ? "40px" : "60px"} ${px}`, background: "#f8f9fa" }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          <h2 style={{ fontFamily: "var(--font-sora), sans-serif", fontSize: isMobile ? "1.2rem" : isSmall ? "1.4rem" : "1.6rem", fontWeight: 800, marginBottom: isMobile ? 20 : 36, textAlign: "center" }}>
          ...
          {[
            ...
            <div key={i} style={{ padding: isMobile ? "14px 0" : "20px 0", borderBottom: i < 3 ? "1px solid #e0e0e0" : "none" }}>
              <div style={{ fontWeight: 700, fontSize: isMobile ? 13 : 15, marginBottom: 8 }}>{"📌 " + faq.q}</div>
              <div style={{ color: "#555", fontSize: isMobile ? 13 : 14, lineHeight: 1.7 }}>{faq.a}</div>
            </div>
          ))}
        </div>
      </section>
