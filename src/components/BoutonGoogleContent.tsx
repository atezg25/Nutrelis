...
function GoogleButton({ onSuccess }: { onSuccess?: () => void }) {
  ...
  const login = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      setChargement(true);
      try {
        await connexionGoogle(tokenResponse.access_token);
        onSuccess?.();
      } catch (e: any) {
    ...
    onError: () => setErreur(t("google.cancelled")),
  });
  ...
  return (
    <div>
      ...
      >
        ...
        {chargement ? t("google.connecting") : t("google.continueWith")}
      </button>
      {erreur && (
...
export default function BoutonGoogleContent({ onSuccess }: { onSuccess?: () => void }) {
  return (
